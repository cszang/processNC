...
There are also several functions (`cropNC`, `maskNC`, `mergeNC` and `aggregateNC`), which rely on the Climate Data Operators (CDO) software (https://code.mpimet.mpg.de/projects/cdo). This software needs to be installed if you want to use those functions in R. CDO is much faster than the equivalent R-functions, thus the CDO-based functions are considerably faster than the `subsetNC()` and `summariseNC()` functions.
